...
def measure_note_f0(y, ini, fin):
    ...
    a = ini + (fin - ini) // 3
    b = fin - (fin - ini) // 3
    return V.estimate_f0_autocorr(y[a:b], SR, 60.0, 1200.0)
...
def main():
    ...
    # pipeline completo: autotune (Do mayor) + timbre de Miku
    res = A.mikutune(x, SR, key='C', scale='major', strength=1.0, retune_speed=1.0,
                     octave=0, miku_amount=0.9, method='pv_formant', ref_miku=miku_ref)
    auto, miku, info = res['auto'], res['miku'], res['info']

    # --- Métrica 1: afinación (cents) antes vs después, por nota ---
    afin = []
    for (m, f_real), (ini, fin) in zip(truth, limits):
        ...
        afin.append({
            ...
            'cents_antes': round(float(V.cents_error(f_before, f_target)), 1),
            'f_despues': round(float(f_after), 2),
            'cents_despues': round(float(V.cents_error(f_after, f_target)), 1),
        })
