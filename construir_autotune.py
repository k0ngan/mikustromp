...
cells.append(md("## 6. Perillas del autotune\n"
                ...
                "- **OCTAVE:** `\"auto\"` lleva tu voz al **registro de Miku** (sube octavas solo); un "
                "entero fija la octava (`0` = no subir).\n"
                "- **GATE** (0–1): compuerta de silencios (más alto = calla más las pausas/respiración).\n\n"
                "> La voz de entrada se **acondiciona** sola (quita ruido/DC, saca el click de grabación y "
                "sube el nivel si venía floja), así que sirve aunque el micrófono grabe bajo."))
