...
escala musical y le **transfiere el timbre (formantes) de Hatsune Miku**, conservando tu
interpretación (melodía y ritmo). Todo con técnicas clásicas de procesamiento de señales —**sin
machine learning ni deep learning**.
...
- `RETUNE_SPEED` (0–1) — velocidad de enganche (1 = instantáneo, efecto "T-Pain"; bajo = glissando suave).
- `MIKU_AMOUNT` (0–1) — cuánto timbre de Miku se impone.
- `OCTAVE` — transposición por octavas (p. ej. +1 para el registro agudo de Miku).
...
3. **Corregir la afinación** de forma variable en el tiempo con un **phase vocoder** (Dolson 1986) que
   **preserva los formantes**, reconstruyendo por *overlap-add* → **la duración se conserva**.
4. **Poner timbre de Miku**: se estima la **envolvente de formantes** de Miku por **cepstrum** y se
   impone sobre la voz afinada como una máscara espectral `H = env_Miku / env_voz`.
...
*streaming* de latencia cero como un plugin en vivo. Una versión en tiempo real (`sounddevice` / plugin
JUCE) y clonación de timbre con RVC/so-vits/OpenUTAU quedan como **trabajo futuro** (fuera del eje DSP
del ramo).
